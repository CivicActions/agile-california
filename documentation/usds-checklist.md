...
2. The most critical pain point which we heard from multiple foster parents was that they often had to call case workers multiple times, and often wait long periods of time without hearing back from case workers, in order to get vital information such as the visitation status of the child. These delays cause strife between the foster parent, case worker, and the system. We heard that there was no clear method to escalate concerns in the case of an urgent need. Case workers complained about having too large of a workload and not having enough time to address concerns, and often getting the same request over and over again. We spent the bulk of our time creating and refining a messaging system, as having a quick way to send messages would facilitate communication, allow foster parents to have a paper trail showing when they contacted case worker in case of long delays in response times, and would give case workers the ability to point to previous responses so they don't have to duplicate replies. 
...
4. We will install Google Analytics to measure number of site visitors, return visitors (to measure how many users must be finding the website/app useful), and bounce rate (how many users abandoned the system). We also would plan to perform user surveys and direct user observation to watch users navigating through the site and app.
